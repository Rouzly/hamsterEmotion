...
img_sad = cv2.imread("Images/sad.jpg")
img_squint = cv2.imread("Images/squint.jpg")
img_happy = cv2.imread("Images/happy.jpg")
img_angry = cv2.imread("Images/angry.webp")
img_surprised = cv2.imread("Images/surprised.jpg")
img_like = cv2.imread("Images/like.webp")
img_dislike = cv2.imread("Images/dislike.webp")
...
img_happy = cv2.resize(img_happy, (150,150))
img_angry = cv2.resize(img_angry, (150,150))
img_surprised = cv2.resize(img_surprised, (150, 150))
img_like = cv2.resize(img_like,(150,150))
img_dislike = cv2.resize(img_dislike,(150,150))
...
options_hands = HandLandmarkerOptions(
    base_options=python.BaseOptions(model_asset_path='hand_landmarker.task'),  # модель для рук
    running_mode=RunningMode.VIDEO,
    num_hands=1,
...
while(True):
    ...
    if detection_result.face_landmarks:
        if detection_result.face_blendshapes:
            ...
            #обнуленнее переменных для дальнейшей работы
            mouthSmileLeft = 0
            mouthSmileRight = 0
            mouthFrownLeft = 0
            mouthFrownRight = 0
